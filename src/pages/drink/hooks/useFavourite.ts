...
export function useToggleFavourite(drinkId: string) {
  ...
  return useMutation({
    ...
    onMutate: async (favourite) => {
      ...
      const previous = [
        ...queryClient.getQueriesData<Drink>({
          queryKey: ["cocktail"],
        }),
        ...queryClient.getQueriesData<Drink[]>({
          queryKey: ["favourites"],
      ...
      queryClient.setQueriesData<Drink>({ queryKey: ["cocktail"] }, (old) =>
        old && old.id === drinkId ? { ...old, favourite } : old,
      );
      ...
      queryClient.setQueriesData<Drink[]>({ queryKey: ["favourites"] }, (old) =>
        old?.map((drink) =>
          drink.id === drinkId ? { ...drink, favourite } : drink,
        ),
      );
