...
- **Cocktail cards** — [src/components/shared/CocktailCard.tsx](src/components/shared/CocktailCard.tsx) is the shared grid card, typed on `Drink` — the single drink shape every endpoint returns (see [src/types/cocktail.ts](src/types/cocktail.ts)), with ingredients already collapsed into an `ingredients` array server-side. Its optional `action` slot renders *outside* the card's `<Link>`, so an interactive overlay like `FavouriteButton` isn't a button nested in an anchor.
